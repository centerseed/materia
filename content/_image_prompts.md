# 原鑑 Materia 文章配圖 — 生圖 Prompt 包（全 35 篇・跨風格多樣版）

## 使用方式
1. 複製「**共用基底前綴**」→ 接該篇整行（行首標了**風格**，讓 35 張分屬不同視覺類型、不單一）。
2. 基底維持不變（暖色品牌感 + 排除規則）；**風格逐篇不同**。
3. 命名 `article_{slug}.png`，放 `materia/content-preview/img/`。
4. ⚠️ 保留「No people faces / No text / No logos」，寫實不超現實。
5. `✅已生` = 已歸檔，可略過（想重生也行）。

## 共用基底前綴（每張都貼這段）
```
Imagery for a premium Taiwanese health-education website. Warm, wholesome, trustworthy mood,
realistic photography (never surreal, never a collage of floating objects).
No people faces, no body parts, no text, no words, no logos, no watermark. Aspect ratio 16:9.
Style & scene:
```

## 8 種風格（穿插使用）
`木質平擺`暖木俯拍食材 ·`亞麻明亮`奶油亞麻布、清新通透 ·`深色精緻`深色石板/深木、單主體戲劇光 ·`大理石`大理石桌面優雅俯拍 ·`料理成品`一道健康料理/湯品 ·`飲品玻璃杯`果昔/茶/水中補充品 ·`生活物件`書桌/床邊/健身情境(無人臉) ·`明亮白底`白色/無縫背景乾淨單物

---

## 護眼明目
- `article_elder-eye-care` ✅已生：木質平擺 — spinach, kale, marigold, egg yolks and amber softgels on wood.
- `article_kids-eye-care` ✅已生：生活物件 — a child's bento box with greens beside a picture book.
- `article_office-eye-fatigue` ✅已生：生活物件 — laptop corner, tea, glasses, dish of greens on a desk.
- `article_what-is-lutein` ✅已生：亞麻明亮微距 — marigold petals, corn and egg yolk close-up.
- `article_compare-lutein` ✅已生：亞麻明亮 — marigold, corn, leafy greens with a dish of softgels.

## 骨骼關節
- `article_elder-bone-collagen` ✅已生：料理成品 — a bowl of clear fish/bone broth with fish skin and citrus.
- `article_elder-joint-care` ✅已生：木質 45° — cucumber, collagen powder spoon, eggs and capsules.
- `article_sports-recovery`：飲品玻璃杯 — a fresh green recovery smoothie in a tall glass with a banana and a few walnuts on a bright counter.
- `article_what-is-cuberup`：深色精緻 — freshly sliced cucumber and a few amber capsules, close-up on dark slate with a single dramatic light beam.
- `article_what-is-ovoderm`：溫暖情境 — a few clean empty eggshell halves showing the delicate translucent inner membrane, beside a small dish of amber capsules and a soft folded linen cloth on light wood, gentle natural light (premium wellness feel, not a raw broken egg).

## 心血管代謝
- `article_elder-heart-care`：料理成品 — a beautifully plated grilled salmon with whole grains and steamed greens, cozy home meal, side view.
- `article_elder-metabolic-care`：大理石 — a kitchen scale with whole grains, fresh vegetables and a glass of water on a white marble surface, top-down.
- `article_men-vitality`：深色精緻 — black cumin seeds, a salmon fillet and whole grains on dark rustic wood, moody hearty side light.
- `article_what-is-thymoquin`：深色精緻 — black cumin (Nigella sativa) seeds spilling from a spoon beside a dark oil bottle, close-up on dark slate.
- `article_what-is-fishoil`：明亮白底 — a glistening salmon fillet with a lemon wedge on a clean bright white surface.
- `article_compare-fishoil`：亞麻明亮 — a salmon fillet, a lemon wedge and a small dish of golden fish-oil softgels.
- `article_omega3-intake-gap`：料理成品 — a large plate holding only one small piece of fish with lots of empty space, angled, quietly conveying a nutrition gap.
- `article_what-is-fibryxa`：飲品玻璃杯 — a tall glass of water with soluble fiber powder swirling and dissolving, oats beside it, bright and clean.

## 腦力情緒
- `article_elder-brain-care`：深色精緻 — walnuts, blueberries and a single glossy fish-oil softgel, close-up on dark slate, dramatic soft light.
- `article_sleep-relax`：生活物件 — a cozy bedside scene at night: a warm cup of tea, an open book and a small dish of capsules under soft warm lamp light.

## 美容養顏
- `article_women-beauty`：大理石 — citrus slices, collagen powder in a spoon and a glass of water on a white marble surface, fresh spa-like.
- `article_menopause-care`：料理成品 — a balanced wholesome plate with salmon, edamame, leafy greens and citrus, gentle and nourishing.
- `article_what-is-collagen`：深色精緻 — a spoon of collagen powder resting on fish skin, close-up on dark slate.
- `article_compare-collagen`：亞麻明亮 — fish skin, a few eggs and two spoons of collagen powder side by side.

## 免疫呼吸
- `article_elder-immune-care`：木質平擺 — bright citrus fruits, red bell peppers and vitamin C capsules on warm wood.
- `article_kids-immune`：生活物件 — a child's plate arranged with colorful fruits and vegetables in a cheerful pattern.
- `article_allergy-respiratory`：深色精緻 — black cumin seeds, a dark oil bottle and a sprig of fresh herb, close-up on dark slate.
- `article_what-is-spm`：明亮白底 — a salmon fillet with a few fish-oil softgels on a clean bright white surface.

## 婦幼孕產
- `article_kids-omega3`：料理成品 — a kid-friendly plate with a small piece of fish, vegetables and a few walnuts, wholesome and bright.
- `article_kids-gut-fiber`：飲品玻璃杯 — a breakfast bowl of oats topped with fresh fruit beside a small glass of yogurt, bright morning light.
- `article_prenatal-nutrition`：大理石 — leafy greens, a salmon piece, eggs, citrus and a few softgels on a soft white marble surface, gentle top-down.

## 基礎營養
- `article_what-is-vitc`：明亮白底 — a juicy fresh orange slice with a single vitamin C capsule and droplets on bright white, macro.
- `article_vitc-comparison`：亞麻明亮 — fresh oranges, kiwi and red bell pepper with a dish of vitamin C capsules.
- `article_what-is-bcomplex`：深色精緻 — a small bundle of wheat and buckwheat sprouts on dark slate, dramatic side light.
- `article_compare-bcomplex`：木質平擺 — buckwheat sprouts and whole grains with a dish of B-complex capsules on wood.

---
## 想再加變化
- 可換背景：淺木 / 奶油亞麻 / 白大理石 / 深色石板 / 淺灰水泥。
- 可換主體型態：生食材 / 一道料理 / 一杯飲品 / 生活物件情境。避免每篇都「原料＋膠囊碟」。

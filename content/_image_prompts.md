# 原鑑 Materia 文章配圖 — 生圖 Prompt 包（多主題：生活情境 × 食材 × 原料）

## 核心原則（重要）
**不要每張都是「食物＋膠囊」。** 衛教文優先用**生活情境／活動／環境／人物**（真正的健康媒體感）；
食材/原料/膠囊的擺拍，主要留給「原料科普」「比較文」這類需要展示原料的文章。

## 使用方式
1. 複製「**共用基底前綴**」→ 接該篇整行（行首標**類型**）。
2. `✅已生`＝已歸檔可略過。
3. 命名 `article_{slug}.png`，放 `materia/content-preview/img/`。

## 共用基底前綴（每張都貼這段）
```
Editorial photography for a premium Taiwanese health-education website. Warm, authentic, trustworthy mood,
natural daylight, realistic (never surreal, never a floating-object collage), styled with depth and context.
People are welcome when natural and candid (East Asian where relevant); or use lifestyle scenes, environments and everyday objects.
Vary the subject widely — do NOT always show food and supplements.
No text, no words, no logos, no watermark. Aspect ratio 16:9.
Subject & scene:
```

---

## 護眼明目
- `article_elder-eye-care` ✅已生
- `article_kids-eye-care`：人物生活 — a young East Asian child taking a break from a tablet to look out a bright window at green trees outside, gently resting the eyes, warm natural light（改：孩子人物，跟公園玩/書桌讀書區隔）.
- `article_office-eye-fatigue` ✅已生
- `article_what-is-lutein` ✅已生（原料科普·食材）
- `article_compare-lutein` ✅已生（比較·食材）

## 骨骼關節
- `article_elder-bone-collagen` ✅已生
- `article_elder-joint-care` ✅已生
- `article_sports-recovery`：人物生活 — an athletic East Asian person stretching or foam-rolling after a workout (or a runner catching their breath on a trail), dynamic and healthy, natural light（改：運動當下/伸展的人物，跟 SPM 靜態恢復、更年期瑜伽區隔）.
- `article_what-is-cuberup` ✅已生（原料科普·食材）
- `article_what-is-ovoderm` ✅已生（原料科普·食材）

## 心血管代謝
- `article_elder-heart-care` ✅已生（料理成品）
- `article_elder-metabolic-care` ✅已生（大理石食材）
- `article_men-vitality`：人物生活 — an energetic middle-aged East Asian man exercising outdoors on a sunny morning (jogging or hiking on a green trail, or finishing a workout with a water bottle), vigorous, healthy and confident, natural light（改：不要深色食材擺拍，跟黑種草籽油/魚油區隔）.
- `article_what-is-thymoquin` ✅已生（原料科普·食材）
- `article_what-is-fishoil` ✅已生（原料科普·食材）
- `article_compare-fishoil`：環境情境 — a fresh fish market / fishmonger counter with whole salmon and mackerel on crushed ice, clean and bright, a shopper's hand reaching (market lifestyle, not a plated dish).
- `article_omega3-intake-gap`：生活概念 — a busy modern desk with a hurried convenience-store meal (instant noodles or a small takeout box) and a coffee, conveying "hard to eat enough good fats from a busy diet", candid realism.
- `article_what-is-fibryxa`：生活情境 — a bright morning kitchen: a hand stirring fiber powder into a glass of water, oats and fresh fruit on the counter, sunlight.

## 腦力情緒
- `article_elder-brain-care`：生活活動 — an older adult's hands doing a crossword or playing Go/chess with reading glasses and a cup of tea on a wooden table, warm daylight, engaged and sharp.
- `article_sleep-relax`：環境 — a serene bedroom at night: a neatly made bed, a warm bedside lamp, a book and a cup of tea on the nightstand, soft moonlight through sheer curtains, calming.

## 美容養顏
- `article_women-beauty`：生活情境 — a bright vanity / dressing table with a round mirror, a few skincare bottles, fresh flowers, a glass of water and a small dish of capsules, elegant morning routine scene.
- `article_menopause-care`：人物生活 — a calm, confident East Asian woman in her fifties doing gentle yoga or stretching by a sunlit window (or walking in a green park), soft natural light, empowered and serene.
- `article_what-is-collagen`：生活情境 — a beauty-corner still life: collagen powder in a glass being stirred, skincare bottles, a hand mirror and soft flowers on a bright vanity.
- `article_compare-collagen`：亞麻明亮·食材 — fish skin and scales on one side, eggs and eggshells on the other, two spoons of collagen powder and a small dish of capsules on cream linen, comparison spread.

## 免疫呼吸
- `article_elder-immune-care`：環境情境 — a cozy autumn scene by a window: a warm cup of tea with steam, a knitted scarf, a few citrus fruits and a book on the sill, soft seasonal light.
- `article_kids-immune`：人物生活 — a smiling East Asian child happily biting into a fresh orange (or holding a plate of colorful fruits and vegetables) at a sunny kitchen table, healthy and vibrant, conveying good nutrition for everyday immunity（改：孩子＋繽紛蔬果，扣住免疫營養，跟公園跑跳無關）.
- `article_allergy-respiratory`：環境 — an open window with sheer curtains gently billowing, a green potted plant and a tissue box on the sill, fresh clean morning air, calm.
- `article_what-is-spm`：生活情境 — a calm recovery scene: a person (back view or hands only) resting after a walk/exercise with a towel and a glass of water by a bright window, soft light, conveying the body settling and recovering (no salmon, no dark seeds — differentiate from other articles).

## 婦幼孕產
- `article_kids-omega3`：人物生活 — an East Asian child focused on reading or drawing at a study desk with books and colored pencils, bright window light, concentrated and content.
- `article_kids-gut-fiber`：人物生活 — a cheerful child eating a breakfast of oatmeal and fresh fruit at a kitchen table, candid morning scene.
- `article_prenatal-nutrition`：人物生活 — a serene pregnant East Asian woman gently resting her hands on her belly by a bright window (or a tender flat-lay of prenatal vitamins with baby booties and a soft blanket), warm and hopeful.

## 基礎營養
- `article_what-is-vitc`：明亮清新·食材 — a vibrant cluster of citrus (orange halves, kiwi slices, red bell pepper) with a small dish of vitamin C capsules and water droplets on a bright surface, juicy and abundant.
- `article_vitc-comparison`：亞麻明亮·比較 — different forms of vitamin C shown side by side: a small dish of capsules, a wooden scoop of vitamin C powder, and an effervescent tablet fizzing in a glass of water, with a few orange slices on cream linen（改：呈現「型態比較」，跟 what-is-vitc 的純水果堆區隔）.
- `article_what-is-bcomplex`：生活情境 — a busy office desk in the afternoon: a coffee cup, an open planner, a laptop edge and a bottle of B-complex, conveying energy and focus during a long day.
- `article_compare-bcomplex`：木質平擺·食材 — small bowls of assorted whole grains (buckwheat, oats, brown rice), a few golden wheat ears and a small dish of B-complex capsules on warm wood, layered comparison spread（不要豆芽/芽菜，會很醜）.

---
## 主題分布檢查（避免全是食物）
- 人物/活動：更年期瑜伽、兒童玩耍、兒童讀書、兒童早餐、孕婦、腦力下棋(手)
- 環境/情境：臥室、開窗通風、秋日窗邊、魚市場、忙碌書桌、晨間廚房、梳妝台
- 食材/原料（保留給科普/比較）：葉黃素、魚油、黃瓜、蛋殼膜、膠原比較、維生素C、B群比較、SPM…
